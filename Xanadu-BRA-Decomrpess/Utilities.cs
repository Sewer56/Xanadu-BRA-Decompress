...
internal static class Utilities
{
    ...
    private static readonly char[] _invalid = Path.GetInvalidPathChars().Union(Path.GetInvalidFileNameChars()).ToArray();
    ...
    public static byte[] SubArrayToNullTerminator(this byte[] data, int index)
    {
        ...
        // Read byte.
        while ( (data[index] != 0x00) && (data[index] < 128) && (! _invalid.Contains((char)data[index])))
        {
